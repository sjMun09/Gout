...
2. 배포 시
   - [ ] Flyway 가 V25 실행 → (1) `CREATE INDEX CONCURRENTLY idx_papers_embedding_hnsw` → (2) `DROP INDEX CONCURRENTLY IF EXISTS idx_papers_embedding`.
   - [ ] 두 단계 사이에는 ivfflat/HNSW 두 인덱스가 공존 → 옵티마이저가 더 나은 쪽 선택. 쿼리 공백 구간 없음.
   - [ ] 빌드 실패 시 → 기존 ivfflat 인덱스가 그대로 남아 서비스 영향 없음 (수동 INVALID 인덱스 제거 필요).
   - [ ] 빌드 중 신규 INSERT 는 인덱스에 늦게 반영될 수 있음 (CONCURRENTLY 특성).
3. 배포 후
   - [ ] `SELECT indexname, indexdef FROM pg_indexes WHERE tablename = 'papers';` 로 HNSW 확인 (`idx_papers_embedding_hnsw`).
   - [ ] `pg_size_pretty(pg_relation_size('idx_papers_embedding_hnsw'))` 로 인덱스 사이즈 기록.
   - [ ] `EXPLAIN ANALYZE SELECT ... ORDER BY embedding <=> ... LIMIT 10` 로 Index Scan 사용 확인.

## 롤백

되돌리려면 `V26__papers_embedding_revert_to_ivfflat.sql` 로 역마이그레이션 (순번 V26 으로 통일):

```sql
-- flyway:executeInTransaction=false
-- 역전환도 동일 원칙: 새 ivfflat 먼저 만들고 HNSW 를 나중에 제거.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_papers_embedding
    ON papers USING ivfflat (embedding vector_cosine_ops)
    WITH (lists = 100);
DROP INDEX CONCURRENTLY IF EXISTS idx_papers_embedding_hnsw;
```

(현 시점에는 생성하지 않고, 실제 롤백이 필요해졌을 때 V26 으로 추가.)
